...
| Runtime | Est. tok/s (2B model) | Notes |
|---|---|---|
| bitnet.cpp native (multi-threaded AVX2) | ~55-65 | All cores, 256-bit SIMD |
| bitnet.cpp single-thread | ~10-15 | AVX2 only |
| **This project (WASM SIMD)** | **~15** | 128-bit SIMD, single thread |
| This project + Web Workers (est.) | ~40-60 | SharedArrayBuffer parallelism |
...
│   └── bench/
│       └── benchmark.c      # Benchmark harness (correctness + perf)
├── web/
│   └── index.html           # Browser UI with SIMD detection
├── docker/
│   └── docker-compose.yml   # Emscripten build container
...
- [ ] TL2 kernel: 3 weights per index (1-bit sign + 4-bit index)
- [ ] Full inference pipeline: model loading (GGUF), tokenization, attention, sampling
- [ ] Web Worker parallelism via SharedArrayBuffer
- [ ] Integration API for embedding in web applications
